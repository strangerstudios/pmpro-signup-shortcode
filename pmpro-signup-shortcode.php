...
function pmprosus_pmpro_email_data($data, $email) {
	...
	if ( ! empty( $_SESSION['pmprosus_autogenerated_password'] ) && strpos( $email->template, 'checkout_' ) !== false ) {
		$data['user_email'] = sprintf(__('Password: %s, Email: %s', 'pmprosus'), sanitize_text_field( $_SESSION['pmprosus_autogenerated_password'] ), esc_html( $data['user_email'] ) );
	...
	?>
		<?php if(!empty($current_user->ID) && pmpro_hasMembershipLevel($level,$current_user->ID)) { ?>
			<?php
				...
				{
					?>
					<div class="pmpro_message pmpro_alert"><?php esc_html_e('&#91;pmpro_signup&#93; Admin Only Shortcode Alert: You are logged in as an administrator and already have the membership level specified.', 'pmprosus'); ?></div>
					<?php
				}
			?>
		<?php } else { ?>
					...
					<?php if ( ! empty( $current_user->ID ) ) { ?>
						...
						<?php if ( $short !== 'emailonly') { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-username">
								...
								<input id="username" name="username" type="text" class="input" size="30" value="<?php echo esc_attr( $username ); ?>" <?php if( ! empty( $hidelabels ) ) { ?>placeholder="<?php esc_attr_e('Username', 'pmprosus'); ?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<?php if ( $short !== 'emailonly') { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-password">
								<label for="password"><?php esc_html_e('Password', 'pmprosus');?></label>
								<input id="password" name="password" type="password" class="input" size="30" value="" <?php if( ! empty( $hidelabels ) ) { ?> placeholder="<?php esc_attr_e('Password', 'pmprosus');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<?php } else { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-password2">
								<label for="password2"><?php esc_html_e('Confirm Password', 'pmprosus');?></label>
								<input id="password2" name="password2" type="password" class="input" size="30" value="" <?php if( ! empty( $hidelabels ) ) { ?>placeholder="<?php esc_attr_e('Confirm Password', 'pmprosus');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<div class="pmpro_checkout-field pmpro_checkout-field-bemail">
							<label for="bemail"><?php esc_html_e('E-mail Address', 'pmprosus');?></label>
							<input id="bemail" name="bemail" type="email" class="input" size="30" value="<?php echo esc_attr( $bemail ); ?>" <?php if( ! empty( $hidelabels ) ) { ?> placeholder="<?php esc_attr_e('E-mail Address', 'pmprosus');?>"<?php } ?> />
						</div>
						...
						<?php } else { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-bconfirmemail">
								<label for="bconfirmemail"><?php esc_html_e('Confirm E-mail', 'pmprosus');?></label>
								<input id="bconfirmemail" name="bconfirmemail" type="email" class="input" size="30" value="" <?php if( ! empty( $hidelabels ) ) { ?>placeholder="<?php esc_attr_e('Confirm E-mail', 'pmprosus');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<div class="pmpro_hidden">
							<label for="fullname"><?php esc_html_e('Full Name', 'pmprosus');?></label>
							<input id="fullname" name="fullname" type="text" class="input" size="30" value="" /> <strong><?php esc_html_e('LEAVE THIS BLANK', 'pmprosus');?></strong>
						</div>
                ...
                'title' => array(),
					...
					<?php if ( ! empty( $login ) && empty( $current_user->ID ) ) { ?>
						<div class="login-link" style="text-align:center;">
							<a href="<?php echo esc_url( wp_login_url( get_permalink() ) ); ?>"><?php esc_html_e( 'Log In','pmprosus' ); ?></a>
						</div>
					<?php } ?>
...
function pmprosus_plugin_row_meta($links, $file) {
	...
	{
		$new_links = array(
			'<a href="' . esc_url('https://www.paidmembershipspro.com/add-ons/pmpro-signup-shortcode/')  . '" title="' . esc_attr( __( 'View Documentation', 'pmprosus' ) ) . '">' . esc_html__( 'Docs', 'pmprosus' ) . '</a>',
			'<a href="' . esc_url('http://paidmembershipspro.com/support/') . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'pmprosus' ) ) . '">' . esc_html__( 'Support', 'pmprosus' ) . '</a>',
		);
