...
function pmprosus_pmpro_email_data($data, $email) {
	...
	if ( ! empty( $_SESSION['pmprosus_autogenerated_password'] ) && strpos( $email->template, 'checkout_' ) !== false ) {
		$data['user_email'] = sprintf(__('Password: %s, Email: %s', 'pmpro-signup-shortcode'), $_SESSION['pmprosus_autogenerated_password'], $data['user_email']);
	...
	?>
		<?php if(!empty($current_user->ID) && pmpro_hasMembershipLevel($level,$current_user->ID)) { ?>
			<?php
				...
				{
					?>
					<div class="pmpro_message pmpro_alert"><?php _e( '&#91;pmpro_signup&#93; Admin Only Shortcode Alert: You are logged in as an administrator and already have the membership level specified.', 'pmpro-signup-shortcode' ); ?></div>
					<?php
				}
			?>
		<?php } else { ?>
				...
				<div class="pmpro_checkout-fields">
					...
					<?php if ( ! empty( $current_user->ID ) ) { ?>
						<p id="pmpro_account_loggedin">
							<?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">log out now</a>.', 'pmpro-signup-shortcode'), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>
						</p>
						<?php
					} else { ?>
						...
						<?php if ( $short !== 'emailonly') { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-username">
								...
								<input id="username" name="username" type="text" class="input" size="30" value="<?php echo esc_attr( $username ); ?>" <?php if( ! empty( $hidelabels ) ) { ?>placeholder="<?php _e('Username', 'pmpro-signup-shortcode');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<?php if ( $short !== 'emailonly') { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-password">
								<label for="password"><?php _e('Password', 'pmpro-signup-shortcode');?></label>
								<input id="password" name="password" type="password" class="input" size="30" value="" <?php if( ! empty( $hidelabels ) ) { ?> placeholder="<?php _e('Password', 'pmpro-signup-shortcode');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<?php } else { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-password2">
								<label for="password2"><?php _e('Confirm Password', 'pmpro-signup-shortcode');?></label>
								<input id="password2" name="password2" type="password" class="input" size="30" value="" <?php if( ! empty( $hidelabels ) ) { ?>placeholder="<?php _e('Confirm Password', 'pmpro-signup-shortcode');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<div class="pmpro_checkout-field pmpro_checkout-field-bemail">
							<label for="bemail"><?php _e('E-mail Address', 'pmpro-signup-shortcode');?></label>
							<input id="bemail" name="bemail" type="email" class="input" size="30" value="<?php echo esc_attr( $bemail ); ?>" <?php if( ! empty( $hidelabels ) ) { ?> placeholder="<?php _e('E-mail Address', 'pmpro-signup-shortcode');?>"<?php } ?> />
						</div>
						...
						<?php } else { ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-bconfirmemail">
								<label for="bconfirmemail"><?php _e('Confirm E-mail', 'pmpro-signup-shortcode');?></label>
								<input id="bconfirmemail" name="bconfirmemail" type="email" class="input" size="30" value="" <?php if( ! empty( $hidelabels ) ) { ?>placeholder="<?php _e('Confirm E-mail', 'pmpro-signup-shortcode');?>"<?php } ?> />
							</div>
						<?php } ?>
						...
						<div class="pmpro_hidden">
							<label for="fullname"><?php _e('Full Name', 'pmpro-signup-shortcode');?></label>
							<input id="fullname" name="fullname" type="text" class="input" size="30" value="" /> <strong><?php _e('LEAVE THIS BLANK', 'pmpro-signup-shortcode');?></strong>
						</div>
					...
					<?php if ( ! empty( $login ) && empty( $current_user->ID ) ) { ?>
						<div class="login-link" style="text-align:center;">
							<a href="<?php echo wp_login_url( get_permalink() ); ?>"><?php _e('Log In','pmpro-signup-shortcode'); ?></a>
						</div>
					<?php } ?>
...
function pmprosus_plugin_row_meta($links, $file) {
	...
	{
		$new_links = array(
			'<a href="' . esc_url('https://www.paidmembershipspro.com/add-ons/pmpro-signup-shortcode/')  . '" title="' . esc_attr( __( 'View Documentation', 'pmpro-signup-shortcode' ) ) . '">' . __( 'Docs', 'pmpro-signup-shortcode' ) . '</a>',
			'<a href="' . esc_url('http://paidmembershipspro.com/support/') . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'pmpro-signup-shortcode' ) ) . '">' . __( 'Support', 'pmpro-signup-shortcode' ) . '</a>',
		);
